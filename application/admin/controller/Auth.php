...
class Auth {
    ...
    protected $_config = array(
        ...
        'AUTH_USER'         => 'AdminUser'                   //用户信息表
    );

    public function __construct() {
        $conf = config('AUTH_CONFIG');
        if ($conf) {
    ...
    protected function getGroups($uid) {
        ...
        $user = model($this->_config['AUTH_USER'])->where(array('id' => $uid))->find();
        if($user){
            $groupInfoArr = model($this->_config['AUTH_GROUP'])->where(array('id' => array('in', $user['groupId'])))->select();
    ...
    public function getAuthList($uid) {
        ...
        $rules = collection(model($this->_config['AUTH_RULE'])->where(array('groupId' => array('in', $groups['id']),'status'  => 1))->select())->toArray();
